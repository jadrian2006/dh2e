<script lang="ts">
    ...
    const roleAptitudes = $derived(selected ? getAptitudes(selected.rules ?? []) : []);
    const talentGrants = $derived(selected ? getGrantsOfType(selected.rules ?? [], "talent") : []);

    /** Find the talent Grant RE with choices */
    ...
    /** Index of the talent choice in all Grant REs */
    ...
    /** Fixed talents (single name, no choice) */
    const fixedTalent = $derived(() => {
        const g = talentGrants.find(tg => !tg.options || tg.options.length <= 1);
        return g?.name ?? null;
    });
        ...
        {#if selected}
            <div class="detail-panel">
                ...
                {#if hasTalentChoice}
                                ...
                                {#each g.options as opt, oi}
                                    <label class="talent-choice-label" onmouseenter={(e) => showTalentTooltip(e, opt)}>
                                        <input
                                            ...
                                            name="talent-choice"
                                            value={opt}
                                            checked={oi === 0 ? !(choiceIdx in roleChoices) : roleChoices[choiceIdx] === opt}
                                            onchange={() => {
                                                if (oi === 0) {
                                                    const { [choiceIdx]: _, ...rest } = roleChoices;
                                                    roleChoices = rest;
                                                } else {
                                                    roleChoices = { ...roleChoices, [choiceIdx]: opt };
                                                }
                                            }}
                                        />
                                        {opt}
                    ...
                    {/if}
                {:else if fixedTalent()}
                    <p class="detail-talent"><strong>Talent:</strong> {fixedTalent()}</p>
                {/if}
...
<style lang="scss">
    ...
    .tag-row { display: flex; flex-wrap: wrap; gap: 0.2rem; margin-top: 0.1rem; }
    .tag {
    ...
    .talent-choices { margin-bottom: 0.4rem; border-bottom: 1px solid var(--dh2e-border, #4a4a55); padding-bottom: 0.4rem; }
    .talent-choices-title { font-size: 0.75rem; font-weight: 700; color: var(--dh2e-gold, #c8a84e); text-transform: uppercase; letter-spacing: 0.05em; margin: 0 0 0.3rem; }
    .talent-choice-group { border: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: 0.15rem; }
    .talent-choice-label {
        display: flex; align-items: center; gap: 0.3rem; font-size: 0.8rem;
        color: var(--dh2e-text-primary, #d0cfc8); cursor: pointer;
        input[type="radio"] { accent-color: var(--dh2e-gold, #c8a84e); margin: 0; }
    }
    .info-btn {
        background: none; border: none; color: var(--dh2e-text-secondary, #a0a0a8);
        cursor: pointer; font-size: 0.7rem; padding: 0 2px; opacity: 0.6;
